<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="uploadRules" label-width="165px" style="width:660px;">
      ...
      <el-form-item v-if="!form.mode" label="NCS copy" prop="ncsCopy">
        <el-input v-model="form.ncsCopy" />
      </el-form-item>
      <el-form-item label="Solvent content" prop="solventContent">
        <el-input v-model="form.solventContent" />
      </el-form-item>
      <el-form-item label="Cycle number" prop="cycle">
        <el-input v-model="form.cycle" />
      </el-form-item>
      <el-form-item v-if="form.mode" label="Upload .pdb file" prop="pdb">
        <el-upload
          ref="pdb"
          action
        ...
        >
          ...
          <div slot="tip" class="el-upload__tip">Only .pdb file can be upload, less than 20MB</div>
        </el-upload>
      </el-form-item>
...
import { uploadForm, uploadFile } from '@/api/ipcas'
...
export default {
  ...
  data() {
    ...
    const checkPir = (_rule, _value, callback) => {
    ...
    return {
      ...
      uploadRules: {
        ...
        cycle: [{
          required: true,
          message: 'Please enter the cycle number',
          trigger: 'blur'
        }, {
          pattern: /^\d{1,2}$/,
          message: 'Please enter a valid cycle number',
          trigger: 'blur'
        }],
        pdb: [{
          required: true,
  ...
  methods: {
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          if(this.form.mode){this.form.pdb = this.$refs.pdb.uploadFiles[0].name}
          ...
          uploadForm(this.form).then(() => {
            ...
            this.loading = false
            setTimeout(() => {
              this.$router.push({ path: '/ipcas/history' })
            }, 2000)
          }).catch(() => {
            Message({
